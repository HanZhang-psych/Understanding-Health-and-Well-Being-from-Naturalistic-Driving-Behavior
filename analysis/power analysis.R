...
# effect size for N = 2200 to reach 80% power
# partial r2 = 1/(1+1/f2), assuming no correlation between beta1 and predictors, so essentially beta1 = sqrt(partial r2)
pwrss.t.reg(beta1 = 0.06, beta0 = 0, k = 22, alpha = 0.05, power= .8, alternative = "not equal")

# get a list of betas and their corresponding power
betas = seq(0.03, 0.1, by=0.001)
powers = sapply(betas, function(b) pwrss.t.reg(beta1 = b, beta0 = 0, k = 22, alpha = 0.05, n = 2200, alternative = "not equal")$power)
...
cbind(betas, powers) %>%
  ...
  geom_vline(xintercept = 0.06, linetype="dashed", color = "blue") +
  annotate("text", x = 0.07, y = 0.82, label = "80% Power", color = "red") +
  annotate("text", x = 0.06, y = 0.6, label = "Effect size: 0.06", color = "blue", angle=90) +
  labs(title="Power Analysis for Multiple Regression",
       subtitle="22 Predictors (1 Key + 21 Controls), N = 2220, Alpha = 0.05",
...
# critical effect size for N = 2200 to reach 80% power
# the effect of interest is the difference in slopes
b1 = 0.01
critical_eff = 0.085 # critical effect
b2 = b1 + critical_eff
...
h1_model <- lavaan::sem(p_calc$modelH1, sample.cov = p_calc$Sigma,
                        sample.nobs = 2220, sample.cov.rescale = FALSE)
...
cbind(eff, power) %>%
  ...
  labs(title="Power Analysis for Path Model",
       subtitle="1 Predictor, 2 DVs, N = 2200, Alpha = 0.05",
